 ...
 * File name:	rtc_datetime.c 
 * Brief description:
 *    RTC function for RTC demo
 * Author:	QST AE team
 * Data:	2021-02-05
 * Revision:V0.01
****************************************************************/
...
//static rtc_dtm_hdl_t s_evt_hdl = NULL;
...
static void rtc_datetime_adjust_baseline(struct tm* datetm);
...
/*
static void dt_timer_start(uint8_t task_id, uint32_t intval_ms)
{
  osal_start_timerEx(task_id, RTC_TIMER_DT_EVT, intval_ms);
}*/
...
static void check_default_datetime(void)
{
  ...
  LOG("default_datetime: %d-%d-%d, %d:%d:%d\n",
          datetm.tm_year+1900,
          datetm.tm_mon+1,
          datetm.tm_mday,
          datetm.tm_hour,
          datetm.tm_min,
          datetm.tm_sec);
  rtc_datetime_adjust_baseline(&datetm);
  return;
}
...
void rtc_datetime_sync_handler(void )
{
  uint32_t tick= 0;
  timer_cnt_get(&tick);
...
static void rtc_datetime_adjust_baseline(struct tm* datetm)
{
  ...
  LOG("rtc_datetime_adjust_baseline:\n");
  print_hex((uint8_t*)&s_stm_cfg, sizeof(s_stm_cfg));
}

int rtc_datetime_set(datetime_t dtm)
{
  struct tm tm;
  ...
  LOG("\nrtc_datetime_set:\n");
  LOG("Time is %d-%d-%d, %d:%d:%d\n", dtm.year, dtm.month, dtm.day, dtm.hour, dtm.minutes, dtm.seconds);
  rtc_datetime_adjust_baseline(&tm);
  print_hex((uint8_t*)&s_stm_cfg, sizeof(s_stm_cfg));
  return SUCCESS;
}

int rtc_datetime_diff(const datetime_t* pdtm_base, const datetime_t* pdtm_cmp)
{
...
time_t rtc_datetime_time_t(void)
{
  time_t tm = (time_t)(s_stm_cfg.tm_base / TM_RATE);
  return tm;
}

int rtc_datetime(datetime_t* pdtm)
{
  struct tm * ptm = NULL;
  uint32_t ticks;
  time_t tm = (time_t)(s_stm_cfg.tm_base / TM_RATE);
...
void rtc_datetime_init(void)//rtc_dtm_hdl_t evt_hdl)
{
  check_default_datetime();
  //dt_timer_start(task_id, RTC_DATETIME_SYNC_INTERVAL);
}
