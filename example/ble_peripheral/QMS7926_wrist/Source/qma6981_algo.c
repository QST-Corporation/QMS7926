...
//static int qst_sleep_flag = 0;
static int qst_fifo_data[QST_SLEEP_FIFO_LEN][3];
...
{ 
  qst_acc_data accData_fifo[3];
  int icount, i,j;

  qmaX981_read_acc(accData_fifo);
...
void Timer_1min_timeout_hamdler()
{

  qst_sleep_get_status();
}

void qma6981_read_fifo(void)
{
	int ret;
	unsigned char databuf[192];
	int raw[96]={0};
//	int acc[4]={0};
	int icount, i,j;
	...
	databuf[0] = 0x0e;
	ret = qma6981_read_bytes(0x0e, databuf, 1);
	if(ret == 0x01){
		ret = qma6981_read_bytes(0x0e, databuf, 1);
		if(ret)
		{
			//NRF_LOG_PRINTF("yzqfifo read 0x0e error!\n");
			//printk("yzqfifo read 0x0e error!\n");
			return;
		}
	}
	if((databuf[0]&0x7f) == 32)
	{
		//printk("yzqfifo 0x0e=%d\n", databuf[0]&0x7f);
		//NRF_LOG_PRINTF("yzqfifo 0x0e=%d\n", databuf[0]&0x7f);
		ret = qma6981_read_bytes(0x0b, databuf, 1);
		if(ret == 0x01){
			ret = qma6981_read_bytes(0x0b, databuf, 1);
		}
		//NRF_LOG_PRINTF("yzqfifo 0x0b=%x\n", databuf[0]);
		ret = qma6981_read_bytes(0x3f,databuf, 192);

		for(j=0; j < QST_SLEEP_FIFO_LEN; j++)
		{
			//databuf[0] = 0x3f;
			
			raw[j*3]	= (uint16_t)((databuf[j*6+1]<<2) |( databuf[j*6]>>6));
			raw[j*3+1]	= (uint16_t)((databuf[j*6+3]<<2) |( databuf[j*6+2]>>6));
			raw[j*3+2]	= (uint16_t)((databuf[j*6+5]<<2) |( databuf[j*6+4]>>6));
			...
			{	//because the data is store in binary complement number formation in computer system
				if ( raw[i] == 0x0200 ) //so we want to calculate actual number here
					raw[i]= -512;		//10bit resolution, 512= 2^(10-1)
				else if ( raw[i] & 0x0200 ) //transfor format
				{					//printk("data 0 step %x \n",data[i]);
					raw[i] -= 0x1;			//printk("data 1 step %x \n",data[i]);
					raw[i] = ~raw[i];		//printk("data 2 step %x \n",data[i]);
					raw[i] &= 0x01ff;		//printk("data 3 step %x \n\n",data[i]);
					raw[i] = -raw[i];		
				}
				
			}
			icount=j;
			//printk("yzqfifo %d	%d	%d\n", raw[0], raw[1], raw[2]);
			#ifdef STEP_COUNT
			#ifdef QMA6981_RANGE_8G
			if(SLEECT_XY==1)
		{
			qst_fifo_data[icount][1] = raw[j*3+1]+0x80;
			qst_fifo_data[icount][2] = raw[j*3+2]+0x80;
		}
		else
		{
			qst_fifo_data[icount][0] = raw[j*3]+0x80;
			//qst_fifo_data[icount][1] = raw[j*3+1]-0x80;
			qst_fifo_data[icount][2] = raw[j*3+2]+0x80;
		}
			//#endif
			#else
			//#ifndef HIGHLIGHT
			qst_fifo_data[icount][0] = raw[j*3]-0x20;
			qst_fifo_data[icount][1] = raw[j*3+1]-0x20;
			qst_fifo_data[icount][2] = raw[j*3+2]-0x20;
			//#endif
			#endif
			#endif
			
			#ifdef QMA6981_RANGE_8G
			qst_fifo_acc[icount].x = (float)qst_fifo_data[icount][0]*9.807/64.00;
			qst_fifo_acc[icount].y = (float)qst_fifo_data[icount][1]*9.807/64.00;
			qst_fifo_acc[icount].z = (float)qst_fifo_data[icount][2]*9.807/64.00;
			#else
			qst_fifo_acc[icount].x = (float)qst_fifo_data[icount][0]*9.807/256.00;
			qst_fifo_acc[icount].y = (float)qst_fifo_data[icount][1]*9.807/256.00;
			qst_fifo_acc[icount].z = (float)qst_fifo_data[icount][2]*9.807/256.00;
			#endif
		//NRF_LOG_PRINTF("x:%d,y:%d,z:%d\n",(int)qst_fifo_data[icount][0],(int)qst_fifo_data[icount][1],(int)qst_fifo_data[icount][2]);
		}
...
uint8_t qst_sleep_set(char enable)
{
	...
	//NRF_LOG_PRINTF("qst_sleep_set:%d\n",g_sleep.is_enabled);
