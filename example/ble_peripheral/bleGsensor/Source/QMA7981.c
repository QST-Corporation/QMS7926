...
uint16_t QMA7981_fetch_acc_data(void)
{
  ...
  QST_i2c_read(pi2c, QMA7981_DATA_LOWBYTE_X,reg_data,6);
  QST_i2c_deinit(pi2c);

  raw[0] = (int16_t)(reg_data[1]<<8)|(int16_t)(reg_data[0]);
  raw[1] = (int16_t)(reg_data[3]<<8)|(int16_t)(reg_data[2]);
  raw[2] = (int16_t)(reg_data[5]<<8)|(int16_t)(reg_data[4]);
  acc_data[0] = (int32_t)raw[0]>>2;
  acc_data[1] = (int32_t)raw[1]>>2;
  acc_data[2] = (int32_t)raw[2]>>2;
  acc_data[0] = (acc_data[0]*GRAVITY_EARTH_1000)/(g_qma7981_lsb_1g);
  acc_data[1] = (acc_data[1]*GRAVITY_EARTH_1000)/(g_qma7981_lsb_1g);
  acc_data[2] = (acc_data[2]*GRAVITY_EARTH_1000)/(g_qma7981_lsb_1g);

  //LOG("%s %d,%d,%d\n", __func__, acc_data[0], acc_data[1], acc_data[2]);
  ev.ev = wmi_event;
  ev.size = 6;
  ev.data = acc_data;
...
int QMA7981_config(void* pi2c)
{
  ...
  if(val == QMA7981_RANGE_4G)
    g_qma7981_lsb_1g = 2048;
  else if(val == QMA7981_RANGE_8G)
    g_qma7981_lsb_1g = 1024;
  else if(val == QMA7981_RANGE_16G)
    g_qma7981_lsb_1g = 512;
  else if(val == QMA7981_RANGE_32G)
